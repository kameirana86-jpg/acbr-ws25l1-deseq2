# Create Conda Environment with Miniconda
I am creating a conda environment for DESeq2 data analysis 
with R and Bioconductor in my windows system using Windows 
Subsystem Linux (WSL)

## Code in WSL terminal
- Start the WSL in Windows
- Type the following to create a conda environment for R4.5 
name as R4.5WS
```{cmd}
conda create --name R4.5WS
```

- Activate the R4.5WS conda environment as follows
```{cmd}
conda activate R4.5WS
```
- install R4.5 in the R4.5WS conda environment
  ```{cmd}
conda install code-forge::r-base
```
`conda-forge` is the channel from anaconda.org [conda-forge channel for r4.5.2] (https://anaconda.org/channels/conda-forge/packages/r-base/overview)





